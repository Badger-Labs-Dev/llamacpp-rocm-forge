...
- `run_bench.py`, `generate_viewer_data.py`, and `recommend_settings.py` are inbound CLI adapters/tools at the top level of `benchmark/`, alongside `tests/`. `gguf_metadata.py` and `huggingface_models.py` used to also be top-level standalone CLIs, but their only real caller was ever `run_bench.py` (via `model_resolution.py`) - the CLI entry points (`argparse`, `main()`) were unused in practice, so they were stripped and the modules moved into `adapters/outbound/` as library-only adapters, matching where their I/O (reading GGUF bytes off disk, calling the Hugging Face Hub over HTTP) actually belongs.
